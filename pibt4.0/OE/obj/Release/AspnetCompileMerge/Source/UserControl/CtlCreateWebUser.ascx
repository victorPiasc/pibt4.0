    ...
    <div class="note-list">
        <ul class="items">
           ...
           <ul>
              ...
              <li>- Include letter(a-z) and numbers(0-9)</li>
              <li>- Include one special character</li>
           </ul>           
            <li>A notification email will be sent to your contact Email after the password was changed
                successfully ! Please check your email later.</li>
            <li>You may change username however, it is not required.</li>
        </ul>
    </div>
<%--    <telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
        <script type="text/javascript">
            function btnCreateUser_click(sernder, args) {
                if (args.EventTarget == "<%=btnCreateUser.UniqueID %>") {
                    if ($("#CreateWebUserDialog .error-list").length > 0 || $("#CreateWebUserDialog .alert-list").length > 0) {
                        PopupMsgDialog("CreateWebUserDialog");
                    }
                }
            }
        </script>
