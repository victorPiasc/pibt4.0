    ...
    public class GetCustomerPaymentProfileList
    {
        public static ANetApiResponse Run(String ApiLoginID, String ApiTransactionKey)
        {
            ...
            if (response != null && response.messages.resultCode == messageTypeEnum.Ok)
            {
                ...
                LouACH.Main.theOutput = response.messages.message[0].text + "Number of payment profiles : " + response.totalNumInResultSet;
                for(int profile=0; profile < response.paymentProfiles.Length; profile++)
                {
                    ...
                    LouACH.Main.theOutput = "<p>" + LouACH.Main.theOutput + response.paymentProfiles[profile].customerPaymentProfileId + "</p>";
                }                
            }
            ...
            else
            {
                if (controller.GetErrorResponse().messages.message.Length > 0)
                {
                    ...
                    LouACH.Main.theOutput = "Null response received : " + controller.GetErrorResponse().messages.message[0].text;
                }
            }
